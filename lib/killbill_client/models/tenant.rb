...
module KillBillClient
  module Model
    class Tenant < TenantAttributes
      KILLBILL_API_TENANTS_PREFIX = "#{KILLBILL_API_PREFIX}/tenants".freeze
      KILLBILL_API_TENANTS_NOTIFICATION = "#{KILLBILL_API_TENANTS_PREFIX}/registerNotificationCallback".freeze
      KILLBILL_API_TENANTS_SYSTEM_CONFIGURATION = "#{KILLBILL_API_TENANTS_PREFIX}/uploadPerTenantConfig".freeze
      KILLBILL_API_TENANTS_PLUGIN_PAYMENT_STATE_MACHINE = "#{KILLBILL_API_TENANTS_PREFIX}/uploadPluginPaymentStateMachineConfig".freeze
      ...
      class << self
        ...
        def register_push_notification(cb, user = nil, reason = nil, comment = nil, options = {})
          ...
          post KILLBILL_API_TENANTS_NOTIFICATION.to_s,
               {},
               { cb: cb },
               {
                 user: user,
        ...
        def retrieve_push_notification(options = {})
          require_multi_tenant_options!(options, 'Push notification is only supported in multi-tenant mode')
          get KILLBILL_API_TENANTS_NOTIFICATION.to_s, {}, options
        end

        def delete_push_notification(user = nil, reason = nil, comment = nil, options = {})
          require_multi_tenant_options!(options, 'Push notification is only supported in multi-tenant mode')
          delete KILLBILL_API_TENANTS_NOTIFICATION.to_s,
                 {},
                 {},
                 {
                   user: user,
        ...
        def add_system_configuration(configuration, user = nil, reason = nil, comment = nil, options = {})
          ...
          post KILLBILL_API_TENANTS_SYSTEM_CONFIGURATION.to_s,
               configuration,
               {},
               {
                 content_type: 'text/plain',
        ...
        def retrieve_system_configurations(options = {})
          require_multi_tenant_options!(options, 'System configuration is only supported in multi-tenant mode')
          get KILLBILL_API_TENANTS_SYSTEM_CONFIGURATION.to_s, {}, options
        end

        def delete_system_configurations(user = nil, reason = nil, comment = nil, options = {})
          require_multi_tenant_options!(options, 'System configuration is only supported in multi-tenant mode')
          delete KILLBILL_API_TENANTS_SYSTEM_CONFIGURATION.to_s,
                 {},
                 {},
                 {
                   user: user,
