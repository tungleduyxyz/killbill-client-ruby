    ...
    class Resource
      ...
      class << self
        ...
        def from_response(resource_class, response)
          ...
          when %r{application/octet-stream}
            response.body
          when %r{text/xml}
            response['location'] || response.body
        ...
        def from_json(resource_class, json)
          ...
          if data.is_a? Array
            ...
            data.each do |data_element|
              records << if data_element.is_a? Enumerable
                           instantiate_record_from_json(resource_class, data_element)
                         else
                           # Value (e.g. String)
                           data_element
                         end
        ...
        def instantiate_record_from_json(resource_class, data)
          ...
          kb_ancestors = resource_class.ancestors.reject { |ancestor| @@attribute_names[ancestor.name].nil? }
          data.each do |name, value|
            ...
            unless attr_desc.nil?
              ...
              if attr_desc[:cardinality] == :many && !type.nil? && value.is_a?(Array)
                newValue = value.map do |val|
                  if val.is_a?(Hash)
                    instantiate_record_from_json(type, val)
                  else
                    val
                  end
                end
        ...
        def attribute(name)
          ...
          unless respond_to?(:json_attributes, true)
            (class << self; self; end)
        ...
        def has_many(attr_name, type = nil)
          ...
          @@attribute_names[name][attr_name.to_sym] = { type: type, cardinality: :many }
        end
        ...
        def has_one(attr_name, type = nil)
          ...
          # add it to attribute_names
          @@attribute_names[name] ||= {}
          @@attribute_names[name][attr_name.to_sym] = { type: type, cardinality: :one }
        end
        ...
        def create_alias(new_name, old_name)
          ...
          alias_method :"#{new_name}=", :"#{old_name}=" # setter
        end
      ...
      def to_hash
        ...
        self.class.json_attributes.each do |name|
          value = send(name)
          json_hash[Utils.camelize name, :lower] = _to_hash(value) unless value.nil?
        end
        json_hash
      end
