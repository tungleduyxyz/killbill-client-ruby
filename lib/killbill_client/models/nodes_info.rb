...
module KillBillClient
  module Model
    class NodesInfo < NodeInfoAttributes
      KILLBILL_NODES_INFO_PREFIX = "#{KILLBILL_API_PREFIX}/nodesInfo".freeze
      ...
      class << self
        ...
        def trigger_node_command_wait_for_plugin_command_completion(node_command_type, plugin_key, plugin_version, plugin_props, local_node_only, user, reason, comment, options, timeout_sec, sleep_sec, &)
          ...
          wait_for_plugin_command_completion(node_command_type, plugin_key, timeout_sec, sleep_sec, &)
        end
          ...
          proc do
            ...
            node_infos.each do |info|
              raw_node_res = info.plugins_info.find do |e|
                KillBillClient.log :info, format('NodesInfo  -> check for plugin command completion version_check=%s, state_check=%s', (plugin_version.nil? && e.is_selected_for_start) || plugin_version == e.version, state.nil? || e.state == state) if (e.plugin_key == plugin_key) && KillBillClient.logger
        ...
        def wait_for_plugin_command_completion(command, plugin, timeout_sec, sleep_sec)
          KillBillClient.log :info, format("NodesInfo waiting for command='%s', plugin='%s'", command, plugin) if KillBillClient.logger
          begin
            ...
            KillBillClient.log :warn, format("NodesInfo timeout after %s sec for command='%s', plugin='%s'", timeout_sec, command, plugin) if KillBillClient.logger
            raise e
          end
        end
      end
